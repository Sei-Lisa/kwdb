...
-- Generated by LSL2 Derived Files Generator. Database version: 0.0.20250202000; output module version: 0.0.20231219001
functionSigs = [
    ...
    ("llAdjustDamage",LLVoid,[("index",LLInteger),("damage",LLFloat)],
        "Used in an on_damage event to set a new value for the damage that will be inflicted."),
    ...
    ("llDamage",LLVoid,[("id",LLKey),("damage",LLFloat),("damage_type",LLInteger)],
        "Inflicts damage on the target given by id (agent or object)."),
    ...
    ("llDerezObject",LLInteger,[("id",LLKey),("mode",LLInteger)],
        "Derez or make temporary an object rezzed by this object. Mode can be DEREZ_DIE or DEREZ_MAKE_TEMP. Returns TRUE on success or FALSE on failure."),
    ("llDetachFromAvatar",LLVoid,[],
        "Detaches object from avatar"),
    ("llDetectedDamage",LLList,[("index",LLInteger)],
        "Retrieves damage information for the given index. The result is valid only if called from an on_damage event. Returns a list [float damage, integer damage_type, float original_damage]."),
    ...
    ("llDetectedRezzer",LLKey,[("index",LLInteger)],
        "Returns the key of the object or avatar that rezzed the detected object."),
    ...
    ("llFabs",LLFloat,[("val",LLFloat)],
        "Returns the positive version of val"),
    ("llFindNotecardTextCount",LLKey,[("name",LLString),("pattern",LLString),("options",LLList)],
    ...
    ("llFindNotecardTextSync",LLList,[("name",LLString),("pattern",LLString),("start",LLInteger),("count",LLInteger),("options",LLList)],
    ...
    ("llGetAgentInfo",LLInteger,[("id",LLKey)],
        "Returns an integer bitfield containing the agent information about id.\nReturns AGENT_FLYING, AGENT_ATTACHMENTS, AGENT_SCRIPTED, AGENT_MOUSELOOK, AGENT_SITTING, AGENT_ON_OBJECT, AGENT_AWAY, AGENT_WALKING, AGENT_IN_AIR, AGENT_TYPING, AGENT_CROUCHING, AGENT_BUSY, AGENT_ALWAYS_RUN, AGENT_AUTOPILOT, AGENT_AUTOMATED and/or AGENT_FLOATING_VIA_SCRIPTED_ATTACHMENT."),
    ...
    ("llGetAttachedListFiltered",LLList,[("agent",LLKey),("options",LLList)],
        "Finds the attachments in the specified agent matching the criteria."),
    ...
    ("llGetHealth",LLFloat,[("id",LLKey)],
        "Retrieves the agent or object's health."),
    ...
    ("llGetStartString",LLString,[],
        "Returns the string parameter passed to the object by the rezzer via llRezObjectWithParams."),
    ...
    ("llGiveAgentInventory",LLInteger,[("agent",LLKey),("folder",LLString),("items",LLList),("options",LLList)],
    ...
    ("llMapBeacon",LLVoid,[("region",LLString),("position",LLVector),("options",LLList)],
        "Places a beacon on the owner's viewer, optionally opening and focusing the map."),
    ...
    ("llRezObjectWithParams",LLKey,[("itemname",LLString),("params",LLList)],
    ...
    ("llSetAgentRot",LLVoid,[("orientation",LLRot),("flags",LLInteger)],
        "Rotates the avatar to face the indicated orientation."),
    ...
    ("llTransferOwnership",LLInteger,[("agent",LLKey),("flags",LLInteger),("options",LLList)],
        "Transfer the ownership of the object to the agent, assuming it's inworld and not an attachment.\n- When flags = 0, ownership of the object will change in place.\n- When flags = TRANSFER_FLAG_TAKE, the object will go to the destination agent's inventory and derezzed from inworld.\n- When flags = TRANSFER_FLAG_COPY, a copy of the object will go to the destination agent's inventory, and the original object will remain in place and owned by its original owner.\nReturns TRANSFER_OK on success, or one of the TRANSFER_xxxx error constants on failure."),
