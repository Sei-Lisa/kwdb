...
i = llLinksetDataCountFound(s);
l = llLinksetDataDeleteFound(s,s);
s = s + llGetRenderMaterial(i);
...
i = llListFindListNext(l,l,i);
i = llIsFriend(k);
s = s + llGetInventoryDesc(s);
k = llRezObjectWithParams(s,l);
llSetLinkSitFlags(i,i);
i = llGetLinkSitFlags(i);
s = s + llComputeHash(s,s);
f = llGetCameraAspect();
f = llGetCameraFOV();
s = s + llGetNotecardLineSync(s,i);
v = llWorldPosToHUD(v);
f = llGetHealth(k);
i = llDerezObject(k,i);
i = llGiveAgentInventory(k,s,l,l);
i = llTransferOwnership(k,i,l);
k = llDetectedRezzer(i);
l = llDetectedDamage(i);
k = llFindNotecardTextCount(s,s,l);
l = llFindNotecardTextSync(s,s,i,i,l);
l = llGetAttachedListFiltered(k,l);
s = s + llGetStartString();
llAdjustDamage(i,f);
llDamage(k,f,i);
llMapBeacon(s,v,l);
llSetAgentRot(r,i);
